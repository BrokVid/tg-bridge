...
## Replay-защита (2026-08-25)

- Nonce-кэш реализован (`src/nonce.rs`): in-memory, ключ `(client, signature)`,
  TTL = 2×окно + 5 c. Повтор байт-в-байт запроса внутри окна -> 401
  `tgb: replay detected`. Проверка идёт после rate limiter (429 не сжигает слот).
- Включён по умолчанию (`[server] replay_protection = true`), отключается в конфиге.
  Легитимные ретраи не страдают: клиент пересоздаёт подпись с новым timestamp
  (проверено в tgbridge_client.py и botik: long-poll getUpdates либо меняет offset,
  либо стартует через >=25 c — коллизий ts+body нет).
- Кэш не персистится; после рестарта экспозицией остаётся только окно timestamp.
- Тестов 35 (12 unit + 23 integration).
- **Задеплоено на de-01 (2026-08-25)**: исходники git archive -> ~/tgbridge-src,
  release-сборка на сервере (swap на месте), бинарник заменён, юнит перезапущен.
  Сквозной smoke с ru-01 (deploy/smoke_replay.py): 1-й запрос 200, байт-в-байт
...
decisions.md «Открытые вопросы».

0. Деплой на de-01 текущего билда + сквозной smoke-тест replay-защиты с ru-01.
1a. Multipart passthrough: файлы через /v1/t/* (Content-Type form-data,
    отдельный лимит тела для файлов, Bot API лимит ~50 МБ), тесты + PROTOCOL.md.
1b. allow_passthrough per client — заявлен в ADR-4, в конфиге отсутствует:
    реализовать или убрать упоминание.
1c. «Ретраи read-only методов» в proxy — заявлены в ARCHITECTURE, в коде нет:
    реализовать или убрать из дока.
2. Webhook-режим (Telegram -> мост -> подписанный форворд клиенту): сначала
    ADR (валидация X-Telegram-Bot-Api-Secret-Token, ретраи/очередь без БД,
    per-client endpoint), потом код.
3. TLS-фронт: примеры caddy/nginx в deploy/, раздел в DEPLOY.md, включение на de-01.
4. Интеграция salut66.ru как первого реального потребителя (INTEGRATION.md +
    tgbridge_client.py уже готовы).
5. Релиз: GitHub publish, CHANGELOG, version 1.0.0, тег, CI зелёный.
   /v1/ протокол замораживается с этого момента.

Критерий 1.0: все фичи выше в проде на de-01, сквозные тесты зелёные,
доки не обещают того, чего нет в коде.

## Следующий шаг

Каркас готов и задеплоен. Дальше по мере надобности: TLS-фронт,
